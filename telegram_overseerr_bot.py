...
async def handle_login_input(update: Update, context: ContextTypes.DEFAULT_TYPE):
    telegram_telegram_user_id = update.effective_user.id
    ...
    if context.user_data["login_step"] == "email":
        ...
        msg = await update.message.reply_text("Please enter your Overseerr password:")
        ...
        if session_cookie:
            ...
            if not overseerr_id:
                await update.message.reply_text("❌ Login failed: Invalid user data.")
                return
            ...
            if CURRENT_MODE == BotMode.NORMAL:
                ...
                sessions[str(telegram_telegram_user_id)] = session_data
                save_user_sessions(sessions)
            elif CURRENT_MODE == BotMode.SHARED and telegram_telegram_user_id == load_config()["admin_telegram_id"]:
                save_shared_session(session_data)
                context.application.bot_data["shared_session"] = session_data
            
            await update.message.reply_text(f"✅ Logged in as {user_info.get('displayName', 'Unknown')}!")
        else:
...
async def logout_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if "session_data" in context.user_data:
        session_cookie = context.user_data["session_data"]["cookie"]
        overseerr_logout(session_cookie)
        ...
        if CURRENT_MODE == BotMode.NORMAL:
            save_user_session(telegram_user_id, {})  # Clear session
        elif CURRENT_MODE == BotMode.SHARED and telegram_user_id == 1:
            context.application.bot_data.pop("shared_session_cookie", None)
            context.application.bot_data.pop("shared_telegram_user_id", None)
...
async def manage_users_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if telegram_user_id != config["admin_telegram_id"]:
...
async def settings_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    """
    config = load_config()
    is_admin = update.effective_user.id == config["admin_telegram_id"]
    await show_settings_menu(update, context, is_admin=is_admin)
...
async def handle_mode_switch(query: CallbackQuery, context: ContextTypes.DEFAULT_TYPE):
    global CURRENT_MODE
    telegram_user_id = query.from_user.id
    if context.user_data.get("overseerr_telegram_user_id") != 1:
        await query.edit_message_text("❌ Only the admin can switch modes.")
        return
    
    data = query.data
    if data == "mode_normal":
        CURRENT_MODE = BotMode.NORMAL
        clear_shared_session()  # Clear shared session when leaving Shared mode
    elif data == "mode_admin":
        CURRENT_MODE = BotMode.ADMIN
        clear_shared_session()
    elif data == "mode_shared":
        CURRENT_MODE = BotMode.SHARED
        if "session_data" not in context.user_data:
            await query.edit_message_text("Please log in as the Shared-User first.")
            context.user_data["login_step"] = "email"
            msg = await context.bot.send_message(query.message.chat_id, "Enter the Shared-User's email:")
            context.user_data["login_message_id"] = msg.message_id
            return
        # Save the admin's session as the shared session
        session_data = context.user_data["session_data"]
        context.application.bot_data["shared_session_cookie"] = session_data["cookie"]
        context.application.bot_data["shared_telegram_user_id"] = session_data["overseerr_telegram_user_id"]
        save_shared_session(session_data)
...
async def message_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    telegram_user_id = update.effective_user.id
    text = update.message.text
    logger.info(f"Message handler triggered by user {telegram_user_id}: {text}")
